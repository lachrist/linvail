...
export const applyInternal = (region, target, that, input) => {
  switch (target.type) {
    ...
    case "guest": {
      ...
      if (name) {
        const { oracle } = region;
        if (!hasOwn(oracle, name)) {
          throw new LinvailExecError("Unknown function name for oracle", {
            name,
          });
        }
        /** @type {import("./oracle.d.ts").Oracle} */
        const { apply } = /** @type {any} */ (oracle)[name];
        if (apply) {
          return apply(region, that, input);
        }
      }
 ...
 *   region: import("./region.d.ts").Region,
 *   target: import("./domain.d.ts").Wrapper,
 *   prototype: import("./domain.d.ts").Reference | null,
 * ) => boolean}
 */
 ...
 * @type {(
 *   region: import("./region.d.ts").Region,
 * ) => import("./reflect.d.ts").Reflect}
 */
export const createReflect = (region) => ({
  ...
  getPrototypeOf: (target) => {
    ...
    return prototype && wrapReference(region, prototype);
  },
  setPrototypeOf: (target, prototype) =>
    setInternalPrototype(region, target, prototype && prototype.inner),
