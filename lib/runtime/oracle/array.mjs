...
export const array_apply_oracle_mapping = {
  ...
  "global.Array.prototype.filter": (region, that, input) => {
    ...
    const array = createArraySpecie(region, target, 0);
    let arg_index = 0;
    let res_index = 0;
  ...
  "global.Array.prototype.map": (region, that, input) => {
    ...
    const array = createArraySpecie(region, target, length);
    for (let index = 0; index < length; index++) {
      if (hasInternalProperty(region, target, index)) {
        if (
  ...
  //   const end = clampInteger(region, atExternal(region, input, 1));
  //   const length = clampPositiveInteger(
  //     region,
  //     leaveValue(
  //       region,
  ...
  //   const array = createArraySpecie(region, target_reference, to - from);
  //   for (let index = from; index < to; index++) {
  //     if (
  //       !defineInternalPropertyDescriptor(region, array, index - from, {
  //         __proto__: null,
  //         value: getInternalPropertyValue(region, target_reference, index, target),
  //         writable: true,
  //         enumerable: true,
  //         configurable: true,
  //       })
