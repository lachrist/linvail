...
// Same //
assertClosure(Linvail.same, { name: "same", length: 2 });
assertEqual(Linvail.same(123, 123), false);
{
  const x = 123;
  assertEqual(Linvail.same(x, x), true);
}
...
// Set //
{
  ...
  }, "TypeError");
  {
    ...
    set.forEach(function (...args) {
      ...
      assertEqual(Linvail.same(args[0], args[1]), true);
      assertEqual(args[2], set);
      keys[keys.length++] = args[0];
    }, 123);
    assertEqual(keys.length, 2);
    assertEqual(Linvail.same(keys[0], k1), true);
    assertEqual(Linvail.same(keys[1], k2), true);
  }
...
// Map //
{
  ...
  }, "TypeError");
  {
    ...
    assertEqual(Linvail.same(entries[1], k1), true);
    assertEqual(entries[0], v1);
    assertEqual(Linvail.same(entries[3], k2), true);
