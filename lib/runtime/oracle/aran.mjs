...
export const aran_apply_oracle_mapping = {
  ...
  "aran.listIteratorRest": (region, _that, input) => {
    ...
    const iterator = toInternalReferenceSloppy(
      region,
      atInternal(region, input, 0),
    );
    const next = toInternalReferenceSloppy(
      region,
    ...
    let index = 0;
    while (!done) {
      const step = applyInternal(region, next, iterator, []);
      if (isInternalPrimitive(region, step)) {
        throw new TypeError(
          "Iterator's next method should return an object at aran.ListIteratorRest",
        );
      }
      done = toBoolean(
        region,
  ...
  "aran.sliceObject": (region, _that, input) => {
    ...
    const source = toInternalReferenceSloppy(
      region,
      atInternal(region, input, 0),
    );
    const exclusion = toInternalReferenceSloppy(
      region,
      atInternal(region, input, 0),
    );
    const result = createObject(
    ...
    for (let index = 0; index < length; index++) {
      ...
      if (!hasInternalOwnProperty(region, exclusion, key)) {
        ...
        if (descriptor && descriptor.enumerable) {
          if (!defineProperty(result, key, descriptor)) {
