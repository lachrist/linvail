...
const applications = [
  ...
  ({
    ...
    (_that, args) =>
      capture(
        setPrototypeOf(
          apply(callee, undefined, args),
          internalizeReference(prototype),
        ),
      ),
  ],
];
...
const compileApply = (context) => {
  ...
  return (callee, that, args) => {
    ...
    if (isPrimitive(callee_value)) {
      return apply(callee_value, that, args);
    } else {
      const external_callee_value = externalizeReference(callee_value);
      const oracle = getMap(
        record,
        /** @type {Function} */ (
          /** @type {unknown} */ (external_callee_value)
        ),
      );
      if (oracle) {
...
const compileConstruct = (context) => {
  ...
  return (callee, args, new_target) => {
    const callee_value = release(callee);
    if (isPrimitive(callee_value)) {
      return capture(construct(callee_value, args, release(new_target)));
    } else {
      const external_callee_value = externalizeReference(callee_value);
      const oracle = getMap(
        record,
...
export const compileCalling = (global, aran, linvail, cage, membrane) => {
  const context = {
    ...
    global: /** @type {import("./global").Global} */ (
      /** @type {unknown} */ (global)
    ),
    aran,
    linvail,
