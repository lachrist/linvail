...
export const object_apply_oracle_mapping = {
  ...
  "global.Object.defineProperties": (region, _that, args) => {
    ...
    for (let index = 0; index < keys.length; index++) {
      const key = keys[index];
      const descriptor = getInternalPropertyValue(
        region,
        descriptors,
        key,
        descriptors,
      ...
      if (
        !defineInternalPropertyDescriptor(
          ...
          target,
          key,
          toInternalDefineDescriptor(region, descriptor),
        )
      ) {
        throw new TypeError(
    ...
    while (!done) {
      ...
      if (!done) {
        ...
        defineProperty(
          ...
          {
            __proto__: null,
            value: getInternalPropertyValue(region, entry, 1, entry),
            writable: true,
            enumerable: true,
            configurable: true,
          },
  ...
  "global.Object.assign": (region, _that, args) => {
    const { "global.TypeError": TypeError } = region;
    const original_target = atInternal(region, args, 0);
    const target = toInternalReferenceSloppy(region, original_target);
    for (let index = 1; index < args.length; index++) {
      const source = toInternalReferenceSloppy(
        region,
        atInternal(region, args, index),
      );
