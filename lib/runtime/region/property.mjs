...
import { enterStandardPrimitive, leaveValue } from "./core.mjs";
...
export const getOwnPlainInternal = (region, target, key) => {
  ...
  if (target.kind === "array") {
    if (isNonLengthPropertyKey(key)) {
      ...
      return {
        ...descriptor,
        value: enterStandardPrimitive(region, descriptor.value),
      };
    }
  } else {
    return getOwnPropertyDescriptor(target.plain, key);
 ...
 */
const toGuestDefineDescriptor = (region, descriptor) => {
...
const sanitizeHostDefineDescriptor = (
  ...
  if (
    ...
    return {
      ...new_descriptor,
      value: enterStandardPrimitive(region, undefined),
    };
  }
  if (
    old_descriptor &&
