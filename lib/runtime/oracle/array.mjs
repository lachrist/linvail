 ...
 */
const convertArrayLike = (region, target, reference, transform, this_arg) => {
  const {
  ...
  const length = Number(
    leaveValue(
      region,
      getInternalPropertyValue(region, reference, "length", target),
    ),
  );
  for (let index = 0; index < length; index++) {
    const item = getInternalPropertyValue(region, reference, index, target);
...
export const array_apply_oracle_mapping = {
  ...
  "global.Array.prototype.at": (region, that, input) => {
    const { "global.Number": Number, "global.undefined": undefined } = region;
    const target = that;
    const target_reference = toInternalReferenceSloppy(region, target);
    let index = Number(atExternal(region, input, 1));
    const length = Number(
      leaveValue(
        region,
  ...
  "global.Array.prototype.pop": (region, that, _input) => {
    ...
    if (length === 0) {
      ...
      deleteInternalOwnProperty(region, target_reference, index);
      return value;
    }
  },
