...
export const array_apply_oracle_mapping = {
  ...
  "global.Array.prototype.reverse": (region, that, _input) => {
    const { "global.Math.floor": floor } = region;
    const target = that;
    const target_reference = toInternalReferenceSloppy(region, target);
    const length = clampPositiveInteger(
      region,
    ...
    for (let index = 0; index < middle; index++) {
      const lower = getInternalPropertyValue(
        region,
        target_reference,
        index,
        target,
      );
      const upper = getInternalPropertyValue(
        region,
        target_reference,
        length - index - 1,
        target,
      );
      setInternalPropertyValue(region, target_reference, index, upper, target);
      setInternalPropertyValue(
        region,
        target_reference,
        length - index - 1,
