 ...
 *   depth: number,
 *   item: import("../domain").InternalValue,
 *   items: import("../domain").InternalReference,
 *   length: number,
 * ) => number}
 */
const flatenArray = (region, depth, item, items, length) => {
 ...
 * ) => import("../domain").PlainInternalArray}
 */
const constructArray = (region, input) => {
  const {
  ...
  if (input.length === 1 && typeof argument0 === "number") {
    ...
    return fromPlainInternalArrayWithExternalPrototype(region, new Array(0));
  } else {
    return fromPlainInternalArrayWithExternalPrototype(
      region,
      apply(toArray, null, input),
    );
  }
};
...
export const array_construct_oracle_mapping = {
  "global.Array": constructArray,
};

/** @type {import("../oracle").ApplyOracleMapping} */
export const array_apply_oracle_mapping = {
  "global.Array": (region, _that, input) => constructArray(region, input),
