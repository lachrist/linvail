...
export const array_apply_oracle_mapping = {
  ...
  "global.Array.prototype.sort": (region, that, input) => {
    ...
    while (!done) {
      for (let index = 0; index < target_length; index++) {
        const has_curr = hasInternalProperty(region, target, index);
        const has_next = hasInternalProperty(region, target, index + 1);
        if (has_curr && has_next) {
          const curr_item = getInternalPropertyValue(
            region,
            target,
            index,
            target,
          );
          const next_item = getInternalPropertyValue(
            region,
            target,
            index + 1,
            target,
          ...
          if (compare) {
            ...
            if (
              String(leaveValue(region, curr_item)) <
              String(leaveValue(region, next_item))
            ) {
              should_swap = true;
            }
          }
