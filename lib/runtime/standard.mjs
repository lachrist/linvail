...
export const toStandardAdvice = ({
  ...
  enterValue,
  leaveValue,
  enterFreshHostClosure: enterRawHostClosure,
  enterHostClosure,
  apply,
  construct,
}) => ({
    ...
    if (isClosureKind(kind)) {
      for (const variable in frame) {
        if (variable === "function.callee") {
          copy[variable] = enterHostClosure(
          ...
          copy[variable] = enterValue(/** @type {any} */ (frame[variable]));
  ...
  "await@after": (_state, value, _tag) =>
    enterValue(/** @type {import("./domain.d.ts").Value} */ (value)),
