 ...
 */
const createPlainInternalObject = (__proto__) =>
  /** @type {any} */ ({ __proto__ });

/**
 * @type {<K extends object, V extends object>(
 *   weakmap: import("../collection").WeakMap<K, V>,
 *   key: K,
...
const compileWeakSetConstructor = (region) => {
  ...
  /**
   * @type {import("../collection").WeakMap<
   *   import("./domain").InternalValue,
   *   import("../collection").WeakSet<import("./domain").InternalValue>
   * >}
   */
  ...
  return enterPseudoClosure(
    ...
    function () {
      ...
      setWeakMap(registery, result, new WeakSet());
      return result;
    },
    {
      kind: "constructor",
...
const compileWeaMapConstructor = (region) => {
  ...
  return enterPseudoClosure(
    ...
    function () {
      ...
      const result = createPlainInternalObject(new.target.prototype);
      setWeakMap(registery, result, new WeakMap());
      return result;
    },
    {
      kind: "constructor",
...
const compileSetConstructor = (region) => {
   ...
   * @type {import("../collection").WeakMap<
   *   import("./domain").InternalValue,
   *   import("../collection").Set<import("./domain").InternalValue>
   * >}
   */
  const registery = new WeakMap();
  const prototype = createPlainInternalObject(
 ...
 * @type {(
 *   region: import("./region").Region,
 *   cache: import("../collection").WeakMap<
...
const compileEventEmitter = (region) => {
  ...
  /**
   * @type {{
   *   capture: import("../collection").WeakMap<
   *     import("./domain").InternalReference,
   *     (primitive: import("./domain").InternalPrimitive) => void
   *   >,
   *   release: import("../collection").WeakMap<
   *     import("./domain").InternalReference,
   *     (primitive: import("./domain").InternalPrimitive) => void
   *   >
   * }}
