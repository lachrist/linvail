 ...
 */
const toInternalReferenceStrict = (region, value) => {
...
const compileWeakSetConstructor = (region) => {
   ...
   * >}
   */
  const registery = createWeakMap([]);
  const prototype = createObject(enterPrototype(region, object_prototype));
  ...
  return enterPseudoClosure(
    ...
    function () {
      ...
      const result = createObject(new.target.prototype);
      /**
       * @type {import("../util/collection").WeakSet<
       *   import("./domain").InternalValue,
       * >}
       */
      const hidden = createWeakSet([]);
...
const compileSetConstructor = (region) => {
  ...
  return enterPseudoClosure(
    ...
    function () {
      ...
      const result = createObject(new.target.prototype);
      setWeakMap(registery, result, createSet([]));
      return result;
    },
    {
      kind: "constructor",
...
const compileMapConstructor = (region) => {
  ...
  return enterPseudoClosure(
    ...
    function () {
      ...
      const result = createObject(new.target.prototype);
      setWeakMap(registery, result, createMap([]));
      return result;
    },
    {
      kind: "constructor",
...
const toEventListenerInner = (region, reference) => {
  const { "global.TypeError": TypeError, "global.Reflect.apply": apply } =
    region;
...
const compileEventEmitter = (region) => {
  const { listeners, "global.undefined": undefined } = region;
  /**
   * @type {{
   ...
   */
  const registery = {
    capture: createWeakMap([]),
    release: createWeakMap([]),
  };
  return {
    addEventListener: enterPseudoClosure(
      ...
      (...args) => {
        ...
        if (listeners[type]) {
          ...
          listeners[type] = createSet([listener]);
        }
        return enterPrimitive(region, undefined);
      },
      {
 ...
 */
export const createLibrary = (region, { dir }) => {
  const {
    "global.Object.create": createObject,
    "global.undefined": undefined,
  ...
  const library = createObject(enterPrototype(region, object_prototype));
