...
export const applyInternal = (region, target, that, input) => {
  ...
  return isGuestInternalReference(region, target)
    ...
    : applyPlainInternalClosure(region, target, that, input);
};
 ...
 */
export const getInternalPrototypeOf = (region, reference) => {
  const {
    global: {
 ...
 * ) => boolean}
 */
export const setInternalPrototypeOf = (region, reference, prototype) => {
  const {
    global: {
 ...
 * ) => (string | symbol)[]}
 */
export const getInternalOwnKeys = (region, reference) => {
  const {
    global: {
      Reflect: { ownKeys },
    },
 ...
 * ) => boolean}
 */
export const hasInternalOwn = (region, target, key) => {
  const {
    global: {
      Object: { hasOwn },
    },
...
export const hasInternal = (region, target, key) => {
  ...
  let current = target;
  while (current !== null) {
    if (hasInternalOwn(region, current, key)) {
      return true;
    }
    current = getInternalPrototypeOf(region, current);
  }
  return false;
};
...
export const getInternal = (region, target, key, receiver) => {
  ...
  let current = target;
  while (current !== null) {
    const descriptor = getInternalOwnPropertyDescriptor(region, current, key);
    if (descriptor) {
      if (isDataDescriptor(descriptor)) {
