...
export const array_apply_oracle_mapping = {
  ...
  "global.Array.prototype.at": (region, that, input) => {
    ...
    let index = atExternal(region, input, 0);
    if (index === undefined) {
      index = 0;
    } else {
      index = clampInteger(region, index);
      if (index < 0) {
        index += length;
      }
    }
  ...
  "global.Array.prototype.fill": (region, that, input) => {
    ...
    let index1 = atExternal(region, input, 1);
    if (index1 === undefined) {
      index1 = 0;
    } else {
      index1 = clampInteger(region, index1);
      if (index1 < 0) {
        index1 += length;
      }
      index1 = max(index1, 0);
    }
    let index2 = atExternal(region, input, 2);
    if (index2 === undefined) {
      index2 = length;
    } else {
      index2 = clampInteger(region, index2);
      if (index2 < 0) {
        index2 += length;
      }
      index2 = min(index2, length);
    }
  ...
  "global.Array.prototype.slice": (region, that, input) => {
    ...
    let index1 = atExternal(region, input, 0);
    if (index1 === undefined) {
      index1 = 0;
    } else {
      index1 = clampInteger(region, index1);
    ...
    let index2 = atExternal(region, input, 1);
    if (index2 === undefined) {
      index2 = length;
    } else {
      index2 = clampInteger(region, index2);
