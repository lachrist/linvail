...
export const toStandardAdvice = ({
  ...
  "program-block@after": (_state, kind, value, _tag) =>
    kind === "deep-local-eval" ? value : unwrap(value),
  "eval@before": (_state, value, _tag) =>
    /** @type {import("./domain.d.ts").Value} */ (
      /** @type {unknown} */ (
        weaveEvalProgram(
          /** @type {import("aran").Program} */ (
            /** @type {unknown} */ (unwrap(value))
          ),
        )
      )
    ),
  "await@before": (_state, value, _tag) => unwrap(value),
  "await@after": (_state, value, _tag) =>
    wrap(/** @type {import("./domain.d.ts").Value} */ (value)),
  "yield@before": (_state, _delegate, value, _tag) => unwrap(value),
  ...
  "export@before": (_state, _specifier, value, _tag) => unwrap(value),
  "import@after": (_state, _source, _specifier, value, _tag) =>
    wrap(/** @type {import("./domain.d.ts").Value} */ (value)),
  "test@before": (_state, _kind, value, _tag) => unwrap(value),
});
