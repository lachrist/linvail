...
export const atExternal = (region, array, index) =>
  index < array.length ? array[index].base : region["global.undefined"];
...
export const toInternalDefineDescriptor = (region, value) => {
  ...
  if (hasInternalProperty(region, value, "get")) {
    descriptor.get = getInternalPropertyValue(region, value, "get", value).base;
  }
  if (hasInternalProperty(region, value, "set")) {
    descriptor.set = getInternalPropertyValue(region, value, "set", value).base;
  }
...
export const getLength = (region, target) => {
  ...
  const length = toInteger(
    Number(getInternalPropertyValue(region, target, "length", target).base),
    0,
  );
...
export const toInternalValueArray = (region, value) => {
  ...
  return toArray(
    {
      length: resolveNaN(
        Number(getInternalPropertyValue(region, value, "length", value).base),
        0,
      ),
    },
...
export const isStrictlyGreater = (region, compare, value1, value2) => {
  ...
  if (compare) {
    ...
    return String(value1.base) > String(value2.base);
  }
};
...
export const convertIterator = (region, iterator, transform, this_arg) => {
  ...
  while (!done) {
    ...
    done = !!getInternalPropertyValue(region, step, "done", iterator).base;
    if (!done) {
