...
import { enterReference } from "./util.mjs";
 ...
 */
export const enterPlainInternalArrayWithExternalPrototype = (region, array) => {
  const {
    "global.Reflect.setPrototypeOf": setPrototypeOf,
    "global.Reflect.getPrototypeOf": getPrototypeOf,
  } = region;
  const prototype = getPrototypeOf(array);
  if (prototype) {
    if (!setPrototypeOf(array, enterReference(region, prototype))) {
