...
module.exports = (access, {membrane:membraneA, objects:objectsA, migrate:forward}, {membrane:membraneB, objects:objectsB, migrate:backward}) => {
  ...
  handlers.apply = (objectA, valueB$, valueB$s) => {
    const valueA$ = membraneA.enter(access[backward](membraneB.leave(valueB$)));
    const valueA$s = [];
    for (let index = 0, length = valueB$s.length; index < length; index++)
      valueA$s[index] = membraneA.enter(access[backward](membraneB.leave(valueB$s[index])));
    return membraneB.enter(access[forward](membraneA.leave(Reflect_apply(objectA, valueA$, valueA$s))));
  };
  ...
  handlers.construct = (objectA, valueB$s, constructorB) => {
    ...
    for (let index = 0, length = valueB$s.length; index < length; index++)
      valueA$s[index] = membraneA.enter(access[backward](membraneB.leave(valueB$s[index])));
    return membraneB.enter(access[forward](membraneA.leave(Reflect_construct(objectA, valueA$s, valueA))));
  };
  ...
  handlers.defineProperty = (objectA, primitive, descriptorB) => {
    ...
    if (Reflect_getOwnPropertyDescriptor(descriptorB, "get") || Reflect_getOwnPropertyDescriptor(descriptorB, "set")) {
      ...
      if (Array_isArray(objectA) && primitive === "length") {
        ...
        descriptorA.value = membraneA.enter(access[backward](membraneB.leave(descriptorB.value)));
      }
    ...
    if (descriptorA) {
      if (Reflect_getOwnPropertyDescriptor(descriptorA, "value")) {
        if (Array_isArray(objectA) && primitive === "length") {
          ...
          descriptorB = {
            value: membraneB.enter(access[forward](membraneA.leave(descriptorA.value))),
