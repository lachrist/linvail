...
exports.TameDescriptorToTameValue = (boolean, $descriptor, access, membrane, sandbox_Object_prototype) => {
  ...
  Reflect_setPrototypeOf($object, access.capture(sandbox_Object_prototype));
  return $object;
};
...
exports.TameValueToTameArguments = ($value, access, membrane) => {
  ...
  const $$value = membrane.taint($value);
  const value = access.release($value);
  const $arguments = {__proto__:null};
