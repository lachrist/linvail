...
module.exports = (membrane, metaof, baseof) => {
  ...
  handlers.apply = (target, value, values) => {
    ...
    return baseof(membrane.leave(Reflect_apply(object, metaof(value), values)));
  };
  ...
  handlers.construct = (target, values) => {
    ...
    for (let index = 0, length = values.length; index < length; index++)
      values[index] = membrane.enter(metaof(values[index]));
    return baseof(membrane.leave(Reflect_construct(object, values)));
  };
    ...
    if (descriptor2) {
      if ("value" in descriptor2) {
        if (descriptor2.writable || descriptor2.configurable)
          return baseof(membrane.leave(descriptor2.value));
        descriptor2.value = baseof(membrane.leave(descriptor2.value));
        Reflect_defineProperty(target, key, descriptor2);
        return descriptor2.value;
      }
      if ("get" in descriptor2)
        return baseof(membrane.leave(Reflect_apply(descriptor2.get, metaof(receiver), [])));
      return void 0;
    }
  ...
  handlers.getOwnPropertyDescriptor = (target, key) => {
    ...
    if (descriptor2) {
      if ("value" in descriptor2) {
        ...
        descriptor2.get = baseof(descriptor2.get);
        descriptor2.set = baseof(descriptor2.set);
      }
  ...
  handlers.getPrototypeOf = (target) => baseof(Reflect_getPrototypeOf(objects.get(target)));
  ...
  handlers.isExtensible = (target) => {
    ...
    if (Reflect_isExtensible(target)) {
      ...
      Reflect_setPrototypeOf(target, baseof(Reflect.getPrototypeOf(object)));
  ...
  handlers.setPrototypeOf = (target, prototype) => Reflect_setPrototypeOf(object, metaof(prototype));
