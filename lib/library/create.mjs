...
import { wrapReference, wrapValue } from "../runtime/region/core.mjs";
import { isStandardPrimitive } from "../runtime/domain.mjs";
import { createEmptyObject } from "../runtime/region/create.mjs";
 ...
 * ) => any}
 */
const compileRefresh = (_region) => {
  /**
   * @type {(
   *   value: import("../runtime/domain.d.ts").Value,
   * ) => import("../runtime/domain.d.ts").Value}
   */
  const refresh = (value) => value;
  return refresh;
};
...
const compileSet = (region) => {
  ...
  const prototype = {
     ...
     */
    add(key) {
      getWeakMapStrict(set_registry, wrapValue(region, this), TypeError).$add(
        wrapValue(region, key),
      );
      return this;
    },
    ...
    forEach(callback, ...rest) {
      ...
      getWeakMapStrict(set_registry, internal_this, TypeError).$forEach(
        (key, val) => {
...
const compileMap = (region) => {
  ...
  const prototype = {
     ...
     */
    set(key, val) {
      getWeakMapStrict(map_registry, wrapValue(region, this), TypeError).$set(
        wrapValue(region, key),
        wrapValue(region, val),
      );
      return this;
    ...
    forEach(callback, ...rest) {
      ...
      const internal_this_arg = wrapValue(region, this_arg);
      const internal_this = wrapValue(region, this);
      getWeakMapStrict(map_registry, internal_this, TypeError).$forEach(
        (val, key) => {
          applyInternal(region, internal_callback, internal_this_arg, [
            val,
            key,
