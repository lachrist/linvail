...
if ("Linvail" in globalThis) {
  const { Linvail } = /** @type {{Linvail: import("../library").Linvail }} */ (
    /** @type {unknown} */ (globalThis)
  );
  /**
   * @type {{
   *   type: "capture" | "release",
   *   data: import("../../util/primitive").Primitive,
   * }[]}
   */
  const stack = [];
  let active = false;
   ...
   *   type: "capture" | "release",
   * ) => (
   *   event: import("../../util/primitive").Primitive,
   * ) => void}
   */
  ...
  Linvail.addEventListener("capture", capture);
  Linvail.addEventListener("release", release);
  active = true;
  typeof 123;
  active = false;
  Linvail.removeEventListener("capture", capture);
  Linvail.removeEventListener("release", release);
  assertEqual(
