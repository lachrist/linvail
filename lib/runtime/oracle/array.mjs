...
export const array_apply_oracle_mapping = {
  ...
  "global.Array.prototype.at": (region, that, input) => {
    ...
    let index = toPositiveInteger(region, atExternal(region, input, 0));
  ...
  "global.Array.prototype.filter": (region, that, input) => {
      ...
      if (hasInternalProperty(region, target_reference, arg_index)) {
        ...
        if (
          ...
          if (
            ...
            throw new TypeError("Cannot define array index during filter");
          }
          res_index++;
        }
      }
    ...
    if (
      ...
      throw new TypeError("Cannot define array length during filter");
    }
    return array;
  },
    ...
    for (let index = 0; index < length; index++) {
      if (hasInternalProperty(region, target_reference, index)) {
        if (
          ...
          throw new TypeError("Cannot define array index during map");
        }
      }
    }
    return array;
