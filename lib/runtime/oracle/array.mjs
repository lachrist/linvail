...
export const array_apply_oracle_mapping = {
  ...
  "global.Array.prototype.unshift": (region, that, input) => {
    ...
    // for (let index = length - 1; index >= 0; index--) {
    //   if (hasInternalProperty(region, target, index)) {
    //     if (length + input.length >= MAX_SAFE_INTEGER) {
    //       throw new TypeError(
    ...
    //     }
    //   } else {
    //     if (!deleteInternalOwnProperty(region, target, index + input.length)) {
    //       throw new TypeError(
    ...
    //     )
    //   ) {
    //     throw new TypeError("Cannot set array index at Array.prototype.unshift");
    //   }
    // }
    // if (
    //   !setInternalPropertyValue(
  ...
  // global.Array.prototype.with >> TODO
