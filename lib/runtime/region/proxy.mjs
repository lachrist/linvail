...
import { enterPrototype, enterReference, enterValue } from "./core.mjs";
...
const guest_proxy_handler = {
  ...
  set: ({ target, region }, key, value, receiver) => {
    ...
    switch (wrapper.type) {
      ...
      case "guest": {
        const descriptor = getOwnPropertyDescriptor(wrapper.base, key);
        if (descriptor) {
          if (isDataDescriptor(descriptor)) {
            if (descriptor.writable) {
              return defineProperty(wrapper.base, key, {
                __proto__: null,
                ...descriptor,
                value,
              });
 ...
 * @type {(
 *   region: import("./region.d.ts").Region,
 *   wrapper: import("../domain.d.ts").HostReferenceWrapper,
 * ) => import("../domain.d.ts").ProxyReference}
 */
const createProxyReference = (region, wrapper) => {
...
export const enterFreshHostReference = (region, kind, fresh) => {
  ...
  const proxy = createProxyReference(region, wrapper);
  wrapper.base = proxy;
