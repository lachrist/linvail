...
}
{
  ...
  assertEqual(collection.has(k1), true);
  assertEqual(collection.has(k2), true);
  assertEqual(collection.has(123), false);
}
...
}
{
  ...
  assertEqual(collection.get(k1), "k1");
  assertEqual(collection.get(k2), "k2");
  assertEqual(collection.get(123), undefined);
}
...

{
  ...
  const symbol1 = Linvail.addEventListener("capture", onCapture);
  const symbol2 = Linvail.addEventListener("release", onRelease);
  assertEqual(Linvail.removeEventListener("capture", symbol1), true);
  assertEqual(Linvail.removeEventListener("release", symbol2), true);
}
