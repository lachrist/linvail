...
export const constructInternal = (region, target, input, new_target) => {
  switch (target.type) {
    ...
    case "host": {
      ...
      return construct(target.plain, input, leaveValue(region, new_target));
    }
    case "guest": {
      const { name } = target;
  ...
  switch (target.type) {
    ...
    case "guest": {
      ...
      if ("value" in host_descriptor) {
        guest_descriptor.value = leaveValue(region, host_descriptor.value);
      }
...
export const getInternalPropertyValue = (region, target, key, receiver) => {
  ...
  while (current !== null) {
    if (current.type === "guest") {
      const { base } = current;
      if (base === array_prototype || base === object_prototype) {
        const descriptor = getOwnPropertyDescriptor(base, key);
        if (descriptor) {
          if (isDataDescriptor(descriptor)) {
        ...
        current = enterPrototype(region, getPrototypeOf(base));
      } else {
        // We have to yield to Reflect.get because the target could be a proxy.
        return enterValue(region, get(base, key, leaveValue(region, receiver)));
      }
    } else {
