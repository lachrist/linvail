...
const visitGeneratorBlock = (node, kind) => ({
  ...
  tail: intercept("unwrap", visitExpression(node.tail), node.tag),
});
...
const visitProgramBlock = (node, kind, hidden) => ({
  ...
  tail:
    kind === "deep-local-eval"
      ...
      : intercept("unwrap", visitExpression(node.tail), node.tag),
});
...
const visitStatement = (node) => {
  switch (node.type) {
    ...
    case "IfStatement": {
      return {
        ...node,
        test: intercept("unwrap", visitExpression(node.test), node.tag),
...
const visitEffect = (node) => {
  switch (node.type) {
    ...
    case "ExportEffect": {
      return {
        ...node,
        value: intercept("unwrap", visitExpression(node.value), node.tag),
      };
    }
    case "WriteEffect": {
      return {
  ...
  switch (node.type) {
    ...
    case "EvalExpression": {
      return {
        ...
        code: intercept(
          "weaveEvalProgram",
          intercept("unwrap", visitExpression(node.code), node.tag),
          node.tag,
        ),
      };
    }
    case "AwaitExpression": {
      return intercept(
        ...
        {
          ...node,
          promise: intercept("unwrap", visitExpression(node.promise), node.tag),
        },
        node.tag,
      );
    }
    case "YieldExpression": {
      return intercept(
        ...
        {
          ...node,
          item: intercept("unwrap", visitExpression(node.item), node.tag),
        },
        node.tag,
      );
    }
