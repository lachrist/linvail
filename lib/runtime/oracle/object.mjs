...
export const object_construct_oracle_mapping = {
  "global.Object": (region, input, new_target) => {
    ...
    if (new_target === enterPlainExternalReference(region, Object)) {
      const internal = atInternal(region, input, 0);
      if (isInternalPrimitive(region, internal)) {
        const external = leavePrimitive(region, internal);
        if (external == null) {
          return createObject(
            enterPlainExternalReference(region, object_prototype),
          );
        } else {
          return enterPlainExternalReference(region, Object(external));
        }
      } else {
        return internal;
      }
...
export const object_apply_oracle_mapping = {
  ...
  "global.Object.entries": (region, _that, input) => {
    ...
    const entries = createArray(region, 0);
    let length = 0;
    for (let index = 0; index < keys.length; index++) {
      const key = keys[index];
      if (typeof key === "string") {
        ...
        if (descriptor && descriptor.enumerable) {
          defineProperty(entries, length, {
            __proto__: null,
            value: fromInternalValueArray(region, [
  ...
  "global.Object.getOwnPropertyNames": (region, _that, input) => {
    ...
    const keys = listInternalOwnPropertyKey(region, target);
    const strings = createArray(region, 0);
    let length = 0;
    for (let index = 0; index < keys.length; index++) {
      const key = keys[index];
      if (typeof key === "string") {
  ...
  "global.Object.groupBy": (region, _that, input) => {
    ...
    let done = false;
    /**
     * @type {{[key in PropertyKey]?: import("../domain.d.ts").PlainInternalArray}}
     */
