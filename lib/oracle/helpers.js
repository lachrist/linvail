...
exports.TameValueToTameDescriptor = (boolean, $value, access, membrane) => {
  ...
  if (hold($value, "value"))
    $descriptor.value = boolean ? access.release(membrane.clean($value.value)) : $value.value;
  if (hold($value, "writable"))
    $descriptor.writable = membrane.clean($value.writable);
  if (hold($value, "get"))
    $descriptor.get = membrane.clean($value.get);
  if (hold($value, "set"))
    $descriptor.set = membrane.clean($value.set);
  if (hold($value, "enumerable"))
    $descriptor.enumerable = membrane.clean($value.enumerable);
  if (hold($value, "configurable"))
    $descriptor.configurable = membrane.clean($value.configurable);
  return $descriptor;
};
